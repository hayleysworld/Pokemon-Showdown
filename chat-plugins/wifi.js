 ...
 * Additional Wisp Edits by: Skyla (Full) ;3;
 **/
'use strict';
...
class QuestionGiveaway extends Giveaway {
	constructor(host, giver, room, prize, question, answers) {
		...
		this.send('<div style="' + GIVEAWAY_DISPLAY + '"><font style="' + GIVEAWAY_TITLE_FONT + '">It\'s giveaway time!</font><br>' + '<font style="' + GIVEAWAY_FLAVOR_FONT + '">Question Giveaway started by ' + Tools.escapeHTML(host.name) + '</font><br>' + '<font style="' + GIVEAWAY_SMALL_FONT + '"><strong>' + Tools.escapeHTML(giver.name) + '</strong> will be giving away a <strong>' + Tools.escapeHTML(this.prize) + '!</strong><br/>' + 'The question will be displayed in one minute! <em>Use /ga to answer.</em></font></div>');
...
class LotteryGiveaway extends Giveaway {
	...
	generateReminder(joined) {
		return '<div style="' + GIVEAWAY_DISPLAY + '"><font style="' + GIVEAWAY_TITLE_FONT + '">It\'s giveaway time!</font><br>' + '<font style="' + GIVEAWAY_FLAVOR_FONT + '">Lottery Giveaway started by ' + Tools.escapeHTML(this.host.name) + '<br>' + '<strong>' + Tools.escapeHTML(this.giver.name) + '</strong> will be giving away: <strong>' + Tools.escapeHTML(this.prize) + '!</strong><br>' + '<font style="' + GIVEAWAY_SMALL_FONT + '">The lottery drawing will occur in 2 minutes, and with ' + this.maxwinners + ' winner' + (this.maxwinners > 1 ? 's' : '') + '!</font><br>' + (joined ? '<button name="send" value="/giveaway leavelottery" style="' + GIVEAWAY_BUTTON + '">Leave</button><br/>' : '<button name="send" value="/giveaway joinlottery" style="' + GIVEAWAY_BUTTON + '">Join</button>') + '</div>';
	}
	display() {
...
let commands = {
	...
	help: function (target, room, user) {
		...
		case 'user':
			if (!this.runBroadcast()) return;
			reply = '<strong>Giveaway participation commands: </strong> (start with /giveaway, except for /ga) <br />' + '- guess or /ga <em>answer</em> - Guesses the answer for a question giveaway<br />' + '- viewanswer - Shows the answer in a question giveaway (only to host/giver)<br />' + '- remind - Shows the details of the current giveaway (can be broadcast)<br />' + '- join or joinlottery - Joins a lottery giveaway<br />' + '- leave or leavelottery - Leaves a lottery giveaway<br />';
			break;
		default:
			if (!this.runBroadcast()) return;
			reply = '<b>Marketplace room Giveaway help and info</b><br />' + '- help user - shows list of participation commands<br />' + '- help staff - shows giveaway staff commands (Requires: % @ # & ~)';
		}
